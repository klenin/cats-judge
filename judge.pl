...
sub generate_test_group
{
    ...
    for (@$tests)
    {
        ...
        $judge->save_input_test_data(
            $pid, $test->{rank},
            $fu->load_file([ $cfg->cachedir, $pid, "$test->{rank}.tst" ], $save_input_prefix)
    ...
    for my $t (@$tests) {
        ...
        if (defined $t->{in_file} && !defined $t->{in_file_size}) {
            ...
            if ($t->{gen_group}) {
                ...
                $judge->save_input_test_data(
                    $pid, $t->{rank},
                    $fu->load_file([ $cfg->cachedir, $pid, "$t->{rank}.tst" ], $problem->{save_input_prefix})
        ...
        if (defined $t->{out_file} && !defined $t->{out_file_size}) {
            ...
            $judge->save_answer_test_data(
                $pid, $t->{rank},
                $fu->load_file([ $cfg->cachedir, $pid, "$t->{rank}.ans" ], $problem->{save_answer_prefix})
